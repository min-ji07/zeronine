...
<%
	...
	int listCount = pi.getListCount4();
...
<body>
	...
	<div style="background: #ffffff;">
		...
		<div id="mainContainer2" style="width: 1024px;height:1000px; margin: 0 auto;">
			<div style="margin-left: 100px; margin-right: 100px; background: rgb(230, 230, 230); padding-bottom: 10px; padding-left: 5px;">
				<span id="itemButton" onclick="tag1Click();"><button id="itemButton2">#<span id="hashName1">악세사리</span></button></span>
				<span id="itemButton" onclick="tag2Click();"><button id="itemButton2">#<span id="hashName2">귀걸이</span></button></span> 
				<span id="itemButton" onclick="tag3Click();"><button id="itemButton2">#<span id="hashName3">팔찌</span></button></span> 
				<span id="itemButton" onclick="tag4Click();"><button id="itemButton2">#<span id="hashName4">크리스탈</span></button></span>
				<!-- <hr style="width: 80%;"> -->
			</div>

			<div id="categoryCounting"
				style="margin-left: 100px; margin-top: 10px; color: black;">
				등록제품 : ( <b style="font-size: 17px;color:rgb(199, 10, 10);"><%=pi.getListCount4()%></b> ) 개
			</div>
	...
	<script>
		$(document).on("click",".JsonPage2",function(){  
 		$.ajax({
 			url: "<%=request.getContextPath()%>/OrderListServlet.do?radio=4&k=2",
             data : {aa :$(this).text()},
             type: "get",
             dataType : 'JSON', 
		...
		$(document).on("click",".JsonPage3",function(){  
	 	    		$.ajax({
	 	    			url: "<%=request.getContextPath()%>/OrderListServlet.do?radio=4&k=3",
	 	                data : {aa :$(this).text()},
	 	                type: "get",
	 	                dataType : 'JSON', 
	 	                success: function (data) {
		...
		$(document).on("click",".JsonPage4",function(){  
	    		$.ajax({
	    			url: "<%=request.getContextPath()%>/OrderListServlet.do?radio=4&k=4",
	                data : {aa :$(this).text()},
	                type: "get",
	                dataType : 'JSON', 
	                success: function (data) {
	...
	function tag1Click(){
		$.ajax({
            url: "<%=request.getContextPath()%>/tagSelect.do?radio=4&hashName="+$('#hashName1').text(),	
            type: "get",
            dataType : 'JSON',                   
            success: function (data) {
            	$('.demo-2').html(data.data);
	...
	function tag2Click(){
		$.ajax({
            url: "<%=request.getContextPath()%>/tagSelect.do?radio=4&hashName="+$('#hashName2').text(),	
            type: "get",
            dataType : 'JSON',                   
            success: function (data) {
            	$('.demo-2').html(data.data);
	...
	function tag3Click(){
		$.ajax({
            url: "<%=request.getContextPath()%>/tagSelect.do?radio=4&hashName="+$('#hashName3').text(),	
            type: "get",
            dataType : 'JSON',                   
            success: function (data) {
            	$('.demo-2').html(data.data);
	...
	function searchButton(){
		$.ajax({
            url: "<%=request.getContextPath()%>/SearchSelect.do?radio=4&search="+$('#search').val(),	
            type: "get",
            dataType : 'JSON',                   
            success: function (data) {
            	$('.demo-2').html(data.data);
