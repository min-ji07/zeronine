...
<%
	...
	int listCount = pi.getListCount3();
...
<body>
	...
	<div style="background: #ffffff;">
		...
		<div id="mainContainer2" style="width: 1024px;height:1000px; margin: 0 auto;">
			<div style="margin-left: 100px; margin-right: 100px; background: rgb(230, 230, 230); padding-bottom: 10px; padding-left: 5px;">
				<span id="itemButton" onclick="tag1Click();"><button id="itemButton2">#<span id="hashName1">애완동물</span></button></span>
				<span id="itemButton" onclick="tag2Click();"><button id="itemButton2">#<span id="hashName2">강아지</span></button></span> 
				<span id="itemButton" onclick="tag3Click();"><button id="itemButton2">#<span id="hashName3">애견용품</span></button></span> 
				<span id="itemButton" onclick="tag4Click();"><button id="itemButton2">#<span id="hashName4">고양이</span></button></span>
				<!-- <hr style="width: 80%;"> -->
			</div>
			<div id="categoryCounting"
				style="margin-left: 100px; margin-top: 10px; color: black;">
				등록제품 : ( <b style="font-size: 17px;color:rgb(199, 10, 10);"><%=pi.getListCount3()%></b> ) 개
			</div>
	...
	<script>
		$(document).on("click",".JsonPage2",function(){  
 		$.ajax({
 			url: "<%=request.getContextPath()%>/OrderListServlet.do?radio=3&k=2",
             data : {aa :$(this).text()},
             type: "get",
             dataType : 'JSON', 
		...
		$(document).on("click",".JsonPage3",function(){  
	 	    		$.ajax({
	 	    			url: "<%=request.getContextPath()%>/OrderListServlet.do?radio=3&k=3",
	 	                data : {aa :$(this).text()},
	 	                type: "get",
	 	                dataType : 'JSON', 
	 	                success: function (data) {
		...
		$(document).on("click",".JsonPage4",function(){  
	    		$.ajax({
	    			url: "<%=request.getContextPath()%>/OrderListServlet.do?radio=3&k=4",
	                data : {aa :$(this).text()},
	                type: "get",
	                dataType : 'JSON', 
	                success: function (data) {
    ...
    function selectEvent(selectObj) {
    	if(selectObj.value == '1'){
    		location.href = "<%=request.getContextPath()%>/selectProductList.do?radio=3&currentPage=1";
    	}
	...
	function tag1Click(){
		$.ajax({
            url: "<%=request.getContextPath()%>/tagSelect.do?radio=3&hashName="+$('#hashName1').text(),	
            type: "get",
            dataType : 'JSON',                   
            success: function (data) {
            	$('.demo-2').html(data.data);
	...
	function tag2Click(){
		$.ajax({
            url: "<%=request.getContextPath()%>/tagSelect.do?radio=3&hashName="+$('#hashName2').text(),	
            type: "get",
            dataType : 'JSON',                   
            success: function (data) {
            	$('.demo-2').html(data.data);
	...
	function tag3Click(){
		$.ajax({
            url: "<%=request.getContextPath()%>/tagSelect.do?radio=3&hashName="+$('#hashName3').text(),	
            type: "get",
            dataType : 'JSON',                   
            success: function (data) {
            	$('.demo-2').html(data.data);
	...
	function tag4Click(){
		$.ajax({
            url: "<%=request.getContextPath()%>/tagSelect.do?radio=3&hashName="+$('#hashName4').text(),	
            type: "get",
            dataType : 'JSON',                   
            success: function (data) {
            	$('.demo-2').html(data.data);
	...
	function searchButton(){
		$.ajax({
            url: "<%=request.getContextPath()%>/SearchSelect.do?radio=3&search="+$('#search').val(),	
            type: "get",
            dataType : 'JSON',                   
            success: function (data) {
            	$('.demo-2').html(data.data);
