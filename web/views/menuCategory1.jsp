...
<%
	...
	int listCount = pi.getListCount1();
...
<body>
	...
	<div style="background: #ffffff;">
		...
		<a id="showHere"></a>
		<div style="width: 1024px; margin: 0 auto; text-align: center; margin-top: 30px;">
		...
		<div id="mainContainer2" style="width: 1024px;height:1000px; margin: 0 auto;">
			<div style="margin-left: 100px; margin-right: 100px; background: rgb(230, 230, 230); padding-bottom: 10px; padding-left: 5px;">
				<span id="itemButton" onclick="tag1Click();"><button id="itemButton2">#<span id="hashName1">티셔츠</span></button></span>
				<span id="itemButton" onclick="tag2Click();"><button id="itemButton2">#<span id="hashName2">아우터</span></button></span> 
				<span id="itemButton" onclick="tag3Click();"><button id="itemButton2">#<span id="hashName3">후드티</span></button></span> 
				<span id="itemButton" onclick="tag4Click();"><button id="itemButton2">#<span id="hashName4">반팔</span></button></span>
				<!-- <hr style="width: 80%;"> -->
			</div>

			<div id="categoryCounting"
				style="margin-left: 100px; margin-top: 10px; color: black;">
				등록제품 : ( <b style="font-size: 17px;color:rgb(199, 10, 10);"><%=pi.getListCount1()%></b> ) 개
			</div>
			...
			<div>
				<select name="sources" id="sources" class="custom-selec" placeholder="- 정렬 선택 -" onchange="javascript:selectEvent(this);"
			...
			<section class="demo-2">
				...
				<div class="box" id="ajaxTable">
					...
					<svg width="100%" height="100%">
						...
						<div id="categoryName">
							<img src="${pageContext.request.contextPath}/resources/productUploadFiles/<%=p.getMainImage()%>">
						</div>
						<div id="categoryid1">
	...
	<script>
		...
		$(document).on("click",".JsonPage3",function(){  
	 	    		$.ajax({
	 	    			url: "<%=request.getContextPath()%>/OrderListServlet.do?radio=1&k=3",
	 	                data : {aa :$(this).text()},
	 	                type: "get",
	 	                dataType : 'JSON', 
	 	                success: function (data) {
		...
		$(document).on("click",".JsonPage4",function(){  
	    		$.ajax({
	    			url: "<%=request.getContextPath()%>/OrderListServlet.do?radio=1&k=4",
	                data : {aa :$(this).text()},
	                type: "get",
	                dataType : 'JSON', 
	                success: function (data) {
    ...
    function selectEvent(selectObj) {
    	if(selectObj.value == '1'){
    		location.href = "<%=request.getContextPath()%>/selectProductList.do?radio=1&currentPage=1";
    	}
	...
	function tag1Click(){
		$.ajax({
            url: "<%=request.getContextPath()%>/tagSelect.do?radio=1&hashName="+$('#hashName1').text(),	
            type: "get",
            dataType : 'JSON',                   
            success: function (data) {
            	$('.demo-2').html(data.data);
	...
	function tag2Click(){
		$.ajax({
            url: "<%=request.getContextPath()%>/tagSelect.do?radio=1&hashName="+$('#hashName2').text(),	
            type: "get",
            dataType : 'JSON',                   
            success: function (data) {
            	$('.demo-2').html(data.data);
	...
	function tag3Click(){
		$.ajax({
            url: "<%=request.getContextPath()%>/tagSelect.do?radio=1&hashName="+$('#hashName3').text(),	
            type: "get",
            dataType : 'JSON',                   
            success: function (data) {
            	$('.demo-2').html(data.data);
	...
	function tag4Click(){
		$.ajax({
            url: "<%=request.getContextPath()%>/tagSelect.do?radio=1&hashName="+$('#hashName4').text(),	
            type: "get",
            dataType : 'JSON',                   
            success: function (data) {
            	$('.demo-2').html(data.data);
	...
	function searchButton(){
		$.ajax({
            url: "<%=request.getContextPath()%>/SearchSelect.do?radio=1&search="+$('#search').val(),	
            type: "get",
            dataType : 'JSON',                   
            success: function (data) {
            	$('.demo-2').html(data.data);
